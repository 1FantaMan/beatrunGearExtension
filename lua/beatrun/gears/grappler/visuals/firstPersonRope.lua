...
-- PostDrawViewModels isn't a real hook; PreDrawEffects is the equivalent
hook.Add("PreDrawEffects", "BeatrunGearsRopeDrawFirstPerson", function()
    ...
    local boneID = vm:LookupBone("ValveBiped.Bip01_L_Hand")
    if not boneID then return end

    -- GetBonePosition() is broken on viewmodels, always returns root pos
    local boneMatrix = vm:GetBoneMatrix(boneID)
    if not boneMatrix then return end

    local rawHandPos = boneMatrix:GetTranslation()

    -- viewmodels render relocated near the eye to avoid clipping, so
    -- reproject the hand's offset onto the player's real eye pos/angles
    local localOffset = WorldToLocal(rawHandPos, angle_zero, vm:GetPos(), vm:GetAngles())
    local anchorPos = LocalToWorld(localOffset, angle_zero, ply:EyePos(), ply:EyeAngles())
    ...
    -- if grappling a moving entity (e.g. a door), track its live position
    -- instead of the fixed point it was at when fired
